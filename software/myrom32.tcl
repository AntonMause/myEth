...
# ########################################################################
# file: myrom32.tcl   (c) 2022 by Anton Mause
#
...
set outfile [open "../hdl/myrom32.vhd" w]
#dbg set outfile [open "myrom32.vhd" w]
set head [open "myrom32_head.vhd" r]
fcopy $head $outfile
close $head
...
while { 1 } {
	...
	if { [string length $two] < 7 } {
        set two $two$one
    } else {
        puts $outfile [format {    x"%-7s%-1s",} $two $one]
        set two ""
	}
}
close $infile
...
set tail [open "myrom32_tail.vhd" r]
fcopy $tail $outfile
close $tail
